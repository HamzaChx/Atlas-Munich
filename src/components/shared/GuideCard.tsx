...
export function GuideCard({ guide, className, showCategory = true }: GuideCardProps) {
  return (
    <Link href={`/guides/${guide.slug}`} className={cn("group block", className)}>
      <div className="relative h-full overflow-hidden rounded-2xl border border-white/10 bg-zinc-900/50 p-5 transition-all duration-300 hover:-translate-y-0.5 hover:border-emerald-500/30 hover:bg-zinc-800/50">
        ...
        <div className="relative">
          ...
          <div className="mb-3 flex flex-wrap items-center gap-2">
            {showCategory && (
              <Badge 
                variant="secondary" 
                className="border-white/10 bg-white/5 font-medium capitalize text-zinc-300"
              >
          ...
          {/* Title */}
          <h3 className="line-clamp-2 text-lg font-semibold leading-snug text-white transition-colors group-hover:text-emerald-400">
            {guide.title}
          </h3>
          ...
          <div className="mt-4 flex items-center justify-between border-t border-white/5 pt-4 text-xs text-zinc-500">
            ...
            <span className="flex items-center font-medium text-emerald-400 transition-transform group-hover:translate-x-0.5">
              Read
