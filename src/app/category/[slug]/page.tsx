...
import { Breadcrumbs, GuideCard, SearchBar, EmptyState, HeroBadge } from "@/components/shared";
...
export default async function CategoryPage({ params }: PageProps) {
  ...
  const gradient = gradientMap[category.key] || { from: "from-emerald-500", to: "to-teal-500", accent: "emerald" };
    ...
    <div className={`min-h-screen ${bgColor}`}>
      ...
      <section className="relative overflow-hidden border-b border-zinc-200 dark:border-white/10">
        ...
        <div className="absolute inset-0 opacity-30 dark:opacity-20">
          <div className={`absolute -left-[20%] top-0 h-[500px] w-[500px] animate-pulse rounded-full bg-gradient-to-br ${gradient.from}/30 ${gradient.to}/30 blur-[120px]`} />
          <div className={`absolute -right-[10%] bottom-0 h-[400px] w-[400px] animate-pulse rounded-full bg-gradient-to-br ${gradient.to}/25 ${gradient.from}/25 blur-[100px]`} style={{ animationDelay: "1s" }} />
          <div className={`absolute left-1/2 top-1/2 h-[300px] w-[300px] -translate-x-1/2 -translate-y-1/2 animate-pulse rounded-full bg-gradient-to-br ${gradient.from}/20 ${gradient.to}/20 blur-[80px]`} style={{ animationDelay: "2s" }} />
        </div>
          ...
          <div className="grid gap-12 lg:grid-cols-2 lg:items-center lg:gap-16">
            ...
            <div>
              ...
              <div className="mb-6 flex items-center gap-3">
                <div className={`inline-flex items-center justify-center rounded-2xl bg-gradient-to-br ${gradient.from} ${gradient.to} p-4 text-white shadow-xl shadow-${gradient.accent}-500/25`}>
                  <IconComponent className="h-8 w-8" />
                </div>
                <div className="flex flex-col">
                  <span className="text-xs font-semibold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">Category</span>
                  <Badge className={`w-fit border-${gradient.accent}-300 dark:border-${gradient.accent}-500/30 bg-${gradient.accent}-100 dark:bg-${gradient.accent}-500/10 text-${gradient.accent}-700 dark:text-${gradient.accent}-300`}>
              ...
              <div className="mt-8 flex flex-wrap items-center gap-6">
                <div className="flex items-center gap-2 rounded-full bg-white/80 dark:bg-zinc-800/80 backdrop-blur-sm px-4 py-2 shadow-sm border border-zinc-200 dark:border-white/10">
                  ...
                  <span className="text-sm font-medium text-zinc-700 dark:text-zinc-300">{categoryGuides.length} Guides</span>
                </div>
                <div className="flex items-center gap-2 rounded-full bg-white/80 dark:bg-zinc-800/80 backdrop-blur-sm px-4 py-2 shadow-sm border border-zinc-200 dark:border-white/10">
                  <Clock className="h-4 w-4 text-blue-600 dark:text-blue-400" />
                  <span className="text-sm font-medium text-zinc-700 dark:text-zinc-300">{totalReadingTime} min read</span>
                </div>
                <div className="flex items-center gap-2 rounded-full bg-white/80 dark:bg-zinc-800/80 backdrop-blur-sm px-4 py-2 shadow-sm border border-zinc-200 dark:border-white/10">
                  <CheckCircle2 className="h-4 w-4 text-amber-600 dark:text-amber-400" />
                  <span className="text-sm font-medium text-zinc-700 dark:text-zinc-300">Community verified</span>
                </div>
              </div>
            ...
            {featuredGuide && (
              <div className="relative">
                ...
                <Card className="relative overflow-hidden border-2 border-zinc-200 dark:border-white/10 bg-white/90 dark:bg-zinc-900/90 backdrop-blur-sm shadow-2xl">
                  {/* Featured label */}
                  <div className={`absolute right-4 top-4 z-10 flex items-center gap-1 rounded-full bg-gradient-to-r ${gradient.from} ${gradient.to} px-3 py-1 text-xs font-semibold text-white shadow-lg`}>
                  ...
                  <div className="p-6 sm:p-8">
                    ...
                    {/* CTA Button */}
                    <Button asChild size="lg" className={`mt-6 w-full bg-gradient-to-r ${gradient.from} ${gradient.to} hover:opacity-90 transition-opacity`}>
            ...
            {allTags.length > 0 && (
              <div className="flex flex-col items-start gap-2 sm:items-end">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {allTags.length > 5 && (
                    <Badge variant="outline" className="border-zinc-200 dark:border-white/10 text-zinc-400">
