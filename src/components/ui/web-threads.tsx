...
// Ported from React Bits (reactbits.dev/backgrounds/web-threads). Renders
// glowing woven sine threads on a WebGL2 canvas via ogl; colors, motion, and
// glow are all driven by props so the hero can hand it the brand palette.
...
const WebThreads: React.FC<WebThreadsProps> = ({
  ...
  useEffect(() => {
    ...
    const renderer = new Renderer({
      ...
      antialias: false,
      // Capped at 1.5 rather than 2: this is a full-screen glow shader, so
      // pixel count matters quadratically, and the extra sharpness past 1.5x
      // is invisible on a soft blurred effect like this.
    ...
    // Dev reloads/HMR churn through WebGL contexts fast enough that the
    // browser can lose one (or refuse a new one) without any JS error being
    // thrown - the canvas just goes blank. Without these, that loss is
    // permanent for the life of the component; with them, the geometry and
    // program (the GPU-side resources actually invalidated by the loss) get
    // rebuilt and the loop picks back up once the driver hands the context
    // back. The renderer/canvas/gl themselves survive a loss, so only the
    // GPU resources need recreating, not the whole component.
    ...
    const onContextRestored = () => {
      ...
      program = new Program(gl, { vertex, fragment, uniforms: program.uniforms });
