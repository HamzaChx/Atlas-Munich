...
const ScrollExpand: React.FC<ScrollExpandProps> = ({
  ...
  // Written in an effect rather than during render: the animation loop below
  // only ever reads this ref from event handlers/rAF callbacks, so it never
  // needs to be current *during* the render itself, and mutating a ref while
  // rendering is what the lint rule (rightly) objects to.
  useEffect(() => {
    propsRef.current = {
      startWidth,
      startHeight,
