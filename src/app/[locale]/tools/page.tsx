...
export default function ToolsPage() {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* ========== MOBILE ROSTER STRIP ========== */}
      <div className="sticky top-(--header-h) z-30 bg-background py-3 shadow-[0_12px_16px_-12px_rgb(0_0_0/0.06)] lg:hidden">
                ...
                <div key={assistant.key} hidden={selected !== assistant.key}>
                  ...
                  >
                    ...
                    {prompts.length > 0 && (
                      <div className="relative mt-7">
                        ...
                        <div className="mt-3 space-y-2">
                          {prompts.map((prompt) =>
                            assistant.chatPath ? (
                              /* Carries the prompt, so the chat opens with the
                                 answer already streaming instead of a blank
                                 composer and the same question to retype. */
                              <Link
                                key={prompt}
                                href={`${assistant.chatPath}?q=${encodeURIComponent(prompt)}`}
                                className="group/prompt flex items-center justify-between gap-3 rounded-2xl bg-card px-4 py-3 text-[14px] leading-snug text-zinc-700 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md dark:text-zinc-200"
                              >
                                {prompt}
                                <ArrowUpRight
                                  className={cn(
                                    "h-4 w-4 shrink-0 opacity-0 transition-opacity duration-200 group-hover/prompt:opacity-100",
                                    assistant.acc
                                  )}
                                  aria-hidden="true"
                                />
                    ...
                    <div className="relative mt-8 flex flex-wrap items-center gap-3">
                      {live ? (
                        /* The primary action opens the tool itself. The landing
                           page is still there for organic search, but it is no
                           longer a toll booth on the way in. */
                        <>
                          <Link
                          ...
                          <Link
                            href={assistant.href!}
                            className="inline-flex items-center rounded-full px-4 py-3 text-sm font-semibold text-zinc-500 underline-offset-4 transition-colors hover:text-zinc-900 hover:underline dark:text-zinc-400 dark:hover:text-zinc-50"
                          >
                            {t("howItWorks")}
                          </Link>
                        </>
